...
void CPU::step()
{
	...
	case 0x3C: {
		...
		registers.f     = (registers.f & CARRY) | ((reg == 0) ? ZERO : 0) |
		              (((old & 0xF) == 0xF) ? HALF_CARRY : 0);
		break;
	}
	...
	case 0x87: {
		...
		registers.f = ((((registers.a & 0xF) + (value & 0xF)) > 0xF) ? HALF_CARRY : 0) |
		              ((registers.a > (0xFF - value)) ? CARRY : 0);
		registers.a += value;
	...
	case 0xcb: {
		...
		case 0x17: {
			...
			reg         = (reg << 1) | (((registers.f & CARRY)) ? 1 : 0);
			registers.f = ((carry) ? CARRY : 0) | ((reg == 0) ? ZERO : 0);
			break;
		}
	...
	default: {
		std::stringstream ss;
		ss << "Unknown instruction: 0x" << std::hex << (int)opcode;
		throw std::runtime_error(ss.str());
	}
	}
}
