...
PPU::PPU(GameBoy &gb) : _gb(gb), dots(0), mode(MODE::OAM)
{
	...
	window = SDL_CreateWindow(reinterpret_cast<char *>(_gb.rom.data() + 0x0134), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, SCREEN_WIDTH, SCREEN_HEIGHT, 0);
		...
		if (dots >= 172)
		{
			...
			for (u8 x = 0; x < GB_SCREEN_WIDTH; x++)
			{
				...
				if (registers[0x00] & 1)
				{
					...
					s8 tile_index = vram[tile_map_area + ((scy + y) / 8) * 32 + ((scx + x) / 8)];
					...
					u8 color_index = (((b >> (8 - tile_x)) & 1) << 1) | ((a >> (8 - tile_x)) & 1);
